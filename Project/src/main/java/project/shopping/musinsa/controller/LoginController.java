...
public class LoginController {
	...
	// 회원가입
		...
		public String loginPOST(Model model, String userId, String userPassword, HttpServletRequest request)
			...
			UserVO vo = userDao.select(userId); // DB에서 아이디 조회 후  정보를 가져옴
			
			if(vo != null) {
				if (userPassword.equals(vo.getUserPassword())) { // 입력한 비밀번호와 DB에서 가져온 비밀번호가 일치하면 로그인 성공
					logger.info("로그인 성공");
					...
					if (targetURL != null) {
						...
						return "redirect:/user/login?result=1" + targetURL;
					} else {
						return "redirect:/user/login?result=1";
					}
				} else {
					logger.info("로그인 실패");
					return "redirect:/user/login?result=0";
				}
			}else {
				return "redirect:/user/login?result=0";			
			}
		...
		public String updatePOST(UserVO vo) {
			...
			if (result == 1) {
				return "redirect:/user/update?userId=" + vo.getUserId() + "&result=" + 1;
			} else {
				return "redirect:/user/update?userId=" + vo.getUserId() + "&result=" + 0;
